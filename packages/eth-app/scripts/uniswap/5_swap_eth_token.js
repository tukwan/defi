...
const ethToBuyToken = web3.utils.toWei('0.05', 'ether') // Buy tokens for 0.05 ETH
...
const encodedABI = tokenExchange.methods.ethToTokenSwapInput(minTokens, deadline).encodeABI()
...
const sendTx = async () => {
  ...
  const txData = {
    ...
    chainId: 4,
    data: encodedABI,
    value: web3.utils.toHex(ethToBuyToken)
  }
...
const getEthToTokenPrice = async () => {
  const result = await tokenExchange.methods.getEthToTokenInputPrice(ethToBuyToken).call()
  const ether = web3.utils.fromWei(ethToBuyToken, 'ether')
  const price = web3.utils.fromWei(result, 'ether')
  const priceRound = Math.round(price * 100) / 100
  console.log(`You will get ${priceRound} SToken for ${ether} ETH`)
}
